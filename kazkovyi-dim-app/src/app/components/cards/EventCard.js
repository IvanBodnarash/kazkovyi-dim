...
export default function EventCard({ title, description, img, date, setShowDetailsModal, setSelectedData, ...props }) {
  ...
  return (
    <div {...props}>
      <div
        className="event-card bg-crema border border-ochre/70 rounded-2xl w-5/6 md:w-94 h-full max-h-120 m-auto p-5 md:p-6 shadow-xl cursor-pointer hover:scale-103 transition-all"
        onClick={() => {
          setSelectedData({
            title,
            description,
      ...
      >
        <div className="bg-ochre p-1 md:p-2 rounded-xl relative">
          <div className="absolute z-10 right-5 bottom-8 rounded-sm bg-cielo/80 py-1 px-3 transition-transform duration-200 ease-out transform-gpu will-change-transform hover:scale-[1.02]">
            {processedDate}
          </div>
          <ImageWithSkeleton
            src={processedImg}
            ...
            height={400}
            alt="eventCardImg"
            className="rounded-xl h-58 w-full md:h-76 md:w-96 object-cover transition-transform duration-300"
          />
        </div>
